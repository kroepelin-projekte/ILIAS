...
class UninstallRemovesMigratedMoFilesTest extends ilLanguageBaseTestCase
{
    ...
    /**
     * True exactly when this test's own call to ensureClientDataDirDefined() defined CLIENT_DATA_DIR
     * itself (as opposed to a pre-existing, foreign definition it merely reused) - see tearDown().
     * With every test running in its own process (see this class' own
    ...
    protected function tearDown(): void
    {
        ...
        // the whole CLIENT_DATA_DIR whenever it was merely defined() - in a full-suite run that
        // constant was the real, pre-existing /var/iliasdata, which got wiped. It must therefore only
        // ever delete a CLIENT_DATA_DIR this test itself created (tracked by
        // $created_client_data_dir_root, set exactly once by ensureClientDataDirDefined() - see there);
        // a foreign, pre-existing definition is left completely untouched, still only ever having its
        // own uniquely-named subdirectory (see overlayFixtureDirectory()) removed, exactly as before.
    ...
    public function testLogsAndSwallowsARemovalFailureForOneModuleWithoutAbortingOthers(): void
    {
        ...
        $shipped_translations = new \ILIAS\Language\ComponentTranslation\Catalog\TranslationCatalog();
