...
class ilLanguageFolderTable implements DataTableInterface\DataRetrieval
{
     ...
     * Constructor
     */
    public function __construct(ilObjLanguageFolder $a_folder, URLBuilder $url_builder, URLBuilderToken $action_token, URLBuilderToken $row_id_token)
    {
        global $DIC;
    ...
    protected function getColums(): array
    {
        $f = $this->ui_factory;
        return [
            'language' => $f->table()->column()->text($this->lng->txt("language"))->withIsSortable(false),
    ...
    public function getRows(
        ...
        foreach ($this->getItems($range, $order) as $idx => $record) {
            ...
            $record['language'] = $record['name'];
            if ($record["status"] ?? false) {
                $record['language'] .= ' (' . $this->lng->txt($record["status"]) . ')';
            }
            ...
            yield $row_builder->buildDataRow($obj_id, $record)
                              ->withDisabledAction('setSystemLanguage', ($record['desc'] === 'not_installed'))
                              ->withDisabledAction('setUserLanguage', ($record['desc'] === 'not_installed'))
                              ->withDisabledAction('refresh', ($record['desc'] === 'not_installed'))
                              ->withDisabledAction('uninstall', ($record['desc'] === 'not_installed'))
                              ->withDisabledAction('edit', ($record['desc'] === 'not_installed'))
